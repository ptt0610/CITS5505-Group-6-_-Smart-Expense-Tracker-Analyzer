        ...
        .then(data => {
          if (data.success) {
              ...
              setTimeout(() => {
                  ...
                  if (uploadedFile && navProfilePic) {
                      const previewURL = URL.createObjectURL(uploadedFile);
                      navProfilePic.src = `${previewURL}?t=${new Date().getTime()}`; // Force refresh
                      console.log("Updated nav profile pic src:", navProfilePic.src);
                      setTimeout(() => URL.revokeObjectURL(previewURL), 5000); // Revoke the object URL
                  } else {
                      console.error("No file selected or navProfilePic not found.");
                  }
                  confirmation.style.display = "none";
              }, 3000);
>>>>>>> Stashed changes
